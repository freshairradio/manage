  ...
  const save = () => {
    api
      .post(
        ...
        {
          ...show,
          slug: show.title.toLowerCase().replace(/[^a-z]+/g, "-"),
          picture: show.picture
            ? `https://cdn.freshair.radio/upload/${show.picture}`
            : null
        }
      )
      .then((s) => {
        goto(`/shows/${show.title.toLowerCase().replace(/[^a-z]+/g, "-")}`);
      });
  };
...
<div class="md:flex md:items-center md:justify-between" in:fade>
  ...
  <div class="mt-4 flex md:mt-0 md:ml-4">
    <span class=" text-gray-700">
      <CloudUpload
        on:click={save}
        tailwind="w-6 h-6 inline ml-1 hover:text-blue-500 cursor-pointer"
      />
    </span>
  </div>
</div>
...
<div class="max-w-3xl mx-auto px-4 sm:px-6 md:px-8">
  ...
  <div class="sm:col-span-1 mt-2">
    <label
      for="image"
      class="block text-sm font-medium leading-5 text-gray-500"> Image </label>
    <div class="mt-1 flex rounded-md shadow-sm">
      <span
        ...
        https://cdn.freshair.radio/upload/
      </span>
      <input
        id="image"
        bind:value={show.picture}
  ...
  <div class="bg-white shadow overflow-hidden sm:rounded-lg mt-2">
    <div class="px-4 py-5 border-b border-gray-200 sm:px-6">
      ...
      <p class="mt-1 max-w-2xl text-sm leading-5 text-gray-500">
        This is displayed on your show page on freshair.radio, your podcast page
        on Spotify and Apple podcasts, and may be used for promotion of your
        show by Freshair
      </p>
    </div>
